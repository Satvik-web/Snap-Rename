...
class SnapRenameTUI(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(id="status-bar"):
            ...
            yield Label("1-6: tools   d: dir   a: apply   u: undo   p: +pipeline   c: clear   r: rm step   q: quit", id="shortcuts-hint")
    ...
    def load_directory(self, folder: Path) -> None:
        ...
        self._refresh_table()
        self._status(
            f"[bold cyan]{len(files)}[/bold cyan] files loaded from "
    ...
    def _update_preview(self) -> None:
        ...
        if current == f.name:
            pw.update(f"[dim]{f.name}[/dim]\n[dim](no change with current settings)[/dim]")
        else:
            pw.update(
                f"[white]{f.name}[/white]\n"
                f"[bright_green]→  {current}[/bright_green]"
            )
    ...
    def _do_add(self) -> None:
        ...
        self._refresh_table()
        self._status(
            f"Added [bold cyan]{label}[/bold cyan]  ·  "
            f"Pipeline: [bold]{n}[/bold] step(s)"
        )
