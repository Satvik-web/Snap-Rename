...
try:
    ...
    from PyQt6.QtGui import QDragEnterEvent, QDropEvent, QColor, QFont, QPixmap, QDrag, QIcon, QDesktopServices, QGuiApplication
...
class MainWindow(QMainWindow):
    def __init__(self, initial_dir=None):
        ...
        if initial_dir and os.path.isdir(initial_dir):
            self.load_workspace(initial_dir)
        else:
    ...
    def load_workspace(self, folder):
        ...
        paths = [Path(folder) / f for f in os.listdir(folder) 
                 if (Path(folder) / f).is_file() and f != ".DS_Store"]  # BUG FIX: filter .DS_Store
...
if __name__ == "__main__":
    ...
    if args.gui:
        ...
        app.setApplicationName("Snap Rename")
        app.setWindowIcon(QIcon("logo.png"))
        QGuiApplication.setApplicationDisplayName("Snap Rename")
        window = MainWindow(initial_dir=args.directory)
        window.show()
        sys.exit(app.exec())
    else:
